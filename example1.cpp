...
int Index = 0;
color4 colors[MAX_NUM_PARTICLES + 24];
...
// url video = https://youtu.be/e8Mwex0bhEI

// Dalam membuat program ini, saya merujuk pada referensi yang tersedia di internet yaitu : http://www.vergenet.net/~conrad/boids/pseudocode.html

// Program ini memodifikasi fungsi idle. Dimana setiap saat mengupdate velocity dari partikel partikel yang ada. Pengubahan kecepatan setiap partikel dengan menambahkan 3 rule yaitu : 

// Rule 1 = Setiap partikel bergerak menuju pusat massa dari sekumpulan partikel tersebut
// Rule 2 = Partikel menjaga jarak kecil dari partikel lain
// Rule 3 = Partikel berusaha untuk mengikuti kecepatan partikel di terdekatnya

vec4 rule1(int p){
    vec4 pcj;

    // mencari pusat massa dari semua partikel yang lain
    // penyederhanaan pusat massa dengan menganggap semua partikel bermassa sama. sehingga hanya perlu diperhatikan posisi nya
    for (int i = 0; i < num_particles; i++ ) {
        if (i != p){
            pcj = pcj + particles[i].position;
        }
    }

    pcj = pcj / (num_particles-1);
    
    // angka 500 digunakan untuk tweak pusat massa
    return (pcj-particles[p].position) / 500;
}
...
vec4 rule2(int p){
    ...
    for (int i = 0; i < num_particles; i++ ) {
        if (i != p){
            // jika partikel terlalu dekat maka posisinya harus di sesuaikan
            if (length(particles[i].position - particles[p].position)<0.05){
                c = c - (particles[i].position - particles[p].position);
            }
        }
    }
    return c;
...
vec4 rule3(int p){
    ...
    // menyesuaikan kecepatan dengan partikel di dekatnya
    for (int i = 0; i < num_particles; i++ ) {
        if (i != p){
            pvj = pvj + particles[i].velocity;
        }
    }

    pvj = pvj / (num_particles -1);

    return (pvj - particles[p].velocity) / 8;
}
...
idle( void )
{
    ...
    //inititate vector and a particle
    vec4 v1, v2, v3;
    ...
    for ( i = 0; i < num_particles; i++ ) {
        ...
        particles[i].velocity = particles[i].velocity + v1 + v2 + v3;
